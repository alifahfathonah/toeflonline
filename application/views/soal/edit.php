                    ...
                    <div class="col-sm-8 col-sm-offset-2">
                        <div class="row">
                            <div class="form-group col-sm-12">
                                <label for="operator_id" class="control-label">Operator (Mata Ujian)</label>
                                <?php if ($this->ion_auth->is_admin()) : ?>
                                    <select required="required" name="operator_id" id="operator_id" class="select2 form-group" style="width:100% !important">
                                        <option value="" disabled selected>Pilih Operator</option>
                                        <?php
                                        $sdm = $soal->operator_id . ':' . $soal->mataujian_id;
                                        foreach ($operator as $d) :
                                            $dm = $d->id_operator . ':' . $d->mataujian_id; ?>
                                            <option <?= $sdm === $dm ? "selected" : ""; ?> value="<?= $dm ?>"><?= $d->nama_operator ?> (<?= $d->nama_mataujian ?>)</option>
                                        <?php endforeach; ?>
                                    </select>
                                    <small class="help-block" style="color: #dc3545"><?= form_error('operator_id') ?></small>
                                <?php else : ?>
                                    <input type="hidden" name="operator_id" value="<?= $operator->id_operator; ?>">
                                    <input type="hidden" name="mataujian_id" value="<?= $operator->mataujian_id; ?>">
                                    <input type="text" readonly="readonly" class="form-control" value="<?= $operator->nama_operator; ?> (<?= $operator->nama_mataujian; ?>)">
                                <?php endif; ?>
                            </div>

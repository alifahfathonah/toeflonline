...
class Soal extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->ion_auth->logged_in()) {
            redirect('auth');
        } else if (!$this->ion_auth->is_admin() && !$this->ion_auth->in_group('operator')) {
            show_error('Hanya Administrator dan operator yang diberi hak untuk mengakses halaman ini, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
        }
    ...
    public function index()
    {
        ...
        if ($this->ion_auth->is_admin()) {
            ...
            $data['mataujian'] = $this->master->getAllMataujian();
        } else {
            //Jika bukan maka mataujian dipilih otomatis sesuai mataujian operator
            $data['mataujian'] = $this->soal->getMataujianOperator($user->username);
        }
    ...
    public function add()
    {
        ...
        if ($this->ion_auth->is_admin()) {
            //Jika admin maka tampilkan semua mataujian
            $data['operator'] = $this->soal->getAllOperator();
        } else {
            //Jika bukan maka mataujian dipilih otomatis sesuai mataujian operator
            $data['operator'] = $this->soal->getMataujianOperator($user->username);
        }
    ...
    public function data($id = null, $operator = null)
    {
        $this->output_json($this->soal->getDataSoal($id, $operator), false);
    }
    ...
    public function save()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($this->ion_auth->is_admin()) {
                $pecah = $this->input->post('operator_id', true);
                $pecah = explode(':', $pecah);
                $data['operator_id'] = $pecah[0];
                $data['mataujian_id'] = end($pecah);
            } else {
                $data['operator_id'] = $this->input->post('operator_id', true);

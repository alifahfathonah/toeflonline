...
class Ujian extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->peserta 	= $this->ujian->getIdPeserta($this->user->username);
	}

	public function akses_operator()
	{
		if (!$this->ion_auth->in_group('operator')) {
			show_error('Halaman ini khusus untuk operator untuk membuat Test Online, <a href="' . base_url('dashboard') . '">Kembali ke menu awal</a>', 403, 'Akses Terlarang');
		}
	}

	public function akses_peserta()
	{
	...
	public function master()
	{
		...
		$data = [
			...
			'judul'	=> 'Ujian',
			'subjudul' => 'Data Ujian',
			'operator' => $this->ujian->getIdOperator($user->username),
		];
	...
	public function add()
	{
		...
		$data = [
			...
			'mataujian'	=> $this->soal->getMataujianOperator($user->username),
			'operator'	=> $this->ujian->getIdOperator($user->username),
		];
	...
	public function validasi()
	{
		...
		$user 	= $this->ion_auth->user()->row();
		$operator 	= $this->ujian->getIdOperator($user->username);
		$jml 	= $this->ujian->getJumlahSoal($operator->id_operator)->jml_soal;
	...
	public function list_json()
	{
		...
		$list = $this->ujian->getListUjian($this->peserta->id_peserta, $this->peserta->kelompok_id);
	...
	public function list()
	{
		...
		$data = [
			...
			'judul'		=> 'Ujian',
			'subjudul'	=> 'List Ujian',
			'peserta'	=> $this->ujian->getIdPeserta($user->username),
		];
		$this->load->view('_templates/dashboard/_header.php', $data);
	...
	public function index()
	{
		...
		$h_ujian 	= $this->ujian->HslUjian($id, $peserta->id_peserta);

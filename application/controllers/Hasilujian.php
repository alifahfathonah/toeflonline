...
class HasilUjian extends CI_Controller
{
	...
	{
		$no_identitas_operator = null;

		if ($this->ion_auth->in_group('operator')) {
			$no_identitas_operator = $this->user->username;
		}

		$this->output_json($this->ujian->getHasilUjian($no_identitas_operator), false);
	}
	...
	public function cetak($id)
	{
		...
		$peserta 	= $this->ujian->getIdPeserta($this->user->username);
		$hasil 	= $this->ujian->HslUjian($id, $peserta->id_peserta)->row();
		$ujian 	= $this->ujian->getUjianById($id);
